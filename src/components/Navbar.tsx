...
const Navbar: React.FC = () => {
  ...
  const NavButton: React.FC<NavButtonProps> = ({ to, icon: Icon, isActive, label }) => {
    return (
      ...
      >
        ...
        <span className={`ml-2 text-xs sm:text-sm ${isActive ? 'block' : 'hidden'} sm:block`}>{label}</span>
      </Link>
    );
  };
  ...
  return (
    <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-50 w-auto">
      <div className="bg-card-primary/30 backdrop-blur-md rounded-full shadow-lg p-1 sm:p-2 flex items-center space-x-1 sm:space-x-2 hover:shadow-glow transition-all duration-300">
        {navItems.map((item) => (
          <NavButton
            key={item.to}
            to={item.to}
